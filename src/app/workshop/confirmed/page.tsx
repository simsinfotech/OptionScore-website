...
export default function WorkshopConfirmedPage() {
  ...
  const hasWhatsapp = !isWsPlaceholder(WORKSHOP.whatsappGroupUrl);
      ...
      <div className="max-w-lg mx-auto px-6 py-16">
        ...
        >
          ...
          <h1 className="font-bold tracking-normal text-3xl text-text-primary mb-3">
            Your Seat Is Confirmed{name ? `, ${name}` : ""}!
          </h1>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  const display = isWsPlaceholder(value) ? "To be shared shortly" : value;
  return (
